...
select * from sales_target;
-- every category achieved monthly target or not (target vs actual)?
select order_details.category , sum(order_details.amount) as total , sales_target.month_of_order_date, sales_target.target,
case
when sum(order_details.amount)<sales_target.target then "target achieved"
else "not achived"
end as status
 from order_details
 inner join sales_target on sales_target.category = order_details.category
group by order_details.category,sales_target.month_of_order_date ,sales_target.target;

-- all category achevied monthly target
...
round((sum(amount)/sum(profit))*100,2) as profit_value , category
from order_details group by category) 
select * from compare where total_sale > (select avg(total_sale) from compare) and
profit_value < (select avg(total_profit) from compare )
order by total_sale desc; 

-- electronic make high sales at low profit
